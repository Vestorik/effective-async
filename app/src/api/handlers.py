...
async def aaa(): return 'hellp world'
    



# # === Пользователи ===
# @api_router.post("/users", status_code=status.HTTP_201_CREATED, response_model=UserOutSheme)
...
#         raise HTTPException(status_code=400, detail=f"Ошибка создания пользователя: {ex}")


# @api_router.get("/users/{user_id}", response_model=UserOutSheme)
# async def get_user(
...
# @api_router.patch("/users/{user_id}", response_model=UserOutSheme)
# async def update_user(
#     user_id: UUID = Path(..., description="ID пользователя"),
#     user_data: UserUpdateSheme = None,
#     user_service: UserService = Depends(get_user_service),
#     db_manager: DataBaseManager = Depends(get_db_manager),
# ):
#     """
